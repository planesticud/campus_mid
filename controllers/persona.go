...
func (c *PersonaController) ActualizarPersona() {
	...
	// alerta que retorna la funcion Actualizar persona
	var alerta models.Alert

	//acumulado de alertas
	//var alertas string

	//almacena resultado de la peticion PUT /persona
	var resultado models.Persona

	//almacena resultado de la peticion PUT /persona_estado_civil
	//var resultado2 models.PersonaEstadoCivil

	//almacena resultado de la peticion PUT /persona_genero
	//var resultado3 models.PersonaGenero
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &persona); err == nil {
		...
		//funcion que realiza  de la  peticion POST /persona
		errPersona := request.SendJson("http://"+beego.AppConfig.String("PersonaService")+"/persona", "PUT", &resultado, persona)
